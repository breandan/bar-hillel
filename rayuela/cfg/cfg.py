...
class CFG:
	...
	def intersect_fsa_ε(self, fsa):
		"""
		New intersection construction that handles ε-transitions
		"""
		assert self.R == fsa.R
		...
		# added S rules to handle ε-transitions
		for qs in product(fsa.Q,repeat=3):
			head, (start, epsilon) = get_intersecting_rule(self.S , (self.S, ε ,), qs)
		...
		# added pre-terminal rules to handle ε-transitions

		for a in self.Sigma:

			for qs in product(fsa.Q, repeat=3):
				h, b = get_intersecting_rule(  a , ( ε , a,) , qs)
